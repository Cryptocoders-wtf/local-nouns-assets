...
const imageUrl = 'https://raw.githubusercontent.com/EibaKatsu/local-nouns-assets/main/images';
const image = (file, baseDir, dir) => {
  return `${imageUrl}/${baseDir}/${dir}/${file}`;
}
      ...
      fs.readdir(assetDir, async (err, dirs) => {
        for (let i = 0; i < dirs.length; i++) {
          ...
          if (dir.match(/^\d{2}-/)) {
            ...
            // console.log("## " + dir)
            pngs.map(png => {
              const line = `${baseDir},${dir},${png},${image(png, baseDir, dir)}`;
              console.log(line);
